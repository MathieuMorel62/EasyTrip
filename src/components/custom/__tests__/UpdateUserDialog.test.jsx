...
beforeAll(() => {
  window.confirm = jest.fn(() => true); // Simule une confirmation
...
describe("UpdateUserDialog", () => {
  ...
  test("test le titre de la page", () => {
    render(<UpdateUserDialog open={true} onOpenChange={mockOnOpenChange} onUpdateSuccess={mockOnUpdateSuccess} />);
  ...
  test("test le message d'erreur si les mots de passe ne correspondent pas", async () => {
    ...
    await waitFor(() => {
      expect(toast).toHaveBeenCalledWith("Les mots de passe ne correspondent pas");
    });
